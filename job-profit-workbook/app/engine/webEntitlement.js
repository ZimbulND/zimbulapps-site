...
import { parseLicenseText, serializePayload, TRIAL_DURATION_MS } from "./licensePayload.js";
...
function view(status, record, now, lastSeen) {
  ...
  return {
    ...
    daysRemaining: remainingMs == null ? null : Math.ceil(remainingMs / (24 * 60 * 60 * 1000)),
    remainingMs,
    message:
      status === "expired"
        ? WEB_EXPIRED_MESSAGE
        : status === "trial"
          ? `Trial: ${Math.ceil(remainingMs / (24 * 60 * 60 * 1000))} day(s) remaining.`
